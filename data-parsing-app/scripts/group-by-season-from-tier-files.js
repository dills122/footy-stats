...
const tierFiles = fs.readdirSync(inputDir).filter(f => f.startsWith('england_tier') && f.endsWith('.csv'));
        ...
        fs.createReadStream(path.join(inputDir, file))
            ...
            .on('end', async () => {
                ...
                for (let i = 0; i < allSeasons.length; i += 25) {
                    ...
                    const outputFile = path.join(outputDir, `england_tier${tier}_${startSeason}-${endSeason}.csv`);
