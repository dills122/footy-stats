...
async function parseInfobox(html) {
    ...
    const table = $('table.infobox').filter((_, el) => {
    ...
    if (relegatedCell.length > 0) {
        ...
        if (text !== 'none') {
            relegated = relegatedCell.find('a').map((_, el) => $(el).text().trim()).get();
        }
    }

    // Handle both "New Club in League" (singular) and "New Clubs in League" (plural)
    const newClubsCell = table.find('th:contains("New Club in League"), th:contains("New Clubs in League")').next('td');
    let newClubs = [];
    if (newClubsCell.length > 0) {
        const text = newClubsCell.text().trim().toLowerCase();
        if (text !== 'none') {
            newClubs = newClubsCell.find('a').map((_, el) => $(el).text().trim()).get();
        }
    }
    ...
    if (foldedCell.length > 0) {
        ...
        if (text !== 'none') {
            folded = foldedCell.find('a').map((_, el) => $(el).text().trim()).get();
        }
    }
    ...
    if (resignedCell.length > 0) {
        ...
        if (text !== 'none') {
            resigned = resignedCell.find('a').map((_, el) => $(el).text().trim()).get();
        }
    }
