...
function constructTier1SeasonResults(tier1SeasonTable, tier2SeasonTable, year, slug) {
    ...
    const tier1RelegatedTeams = tier1SeasonTable.filter(team => team.wasRelegated).map(row => row.team);
    const tier2PromotedTeams = tier2SeasonTable.filter(team => team.wasPromoted).map(row => row.team);
